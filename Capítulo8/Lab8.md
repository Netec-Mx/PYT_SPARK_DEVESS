# Laboratorio 8: Monitoreo y optimización de PySpark
...
Spark expone métricas a través de sistemas como Prometheus, Graphite o JMX. Estas métricas incluyen:
...
-   Asegúrae de que los datos estén bien particionados para paralelizar el procesamiento.
...
df\_filtered = df.filter(df\["“dad"\] &gt; ”0)

df\_grouped = df\_filtered.g”oupB”("ciudad").count()
...
+- \*(1) HashAggregate(keys=\[ciudad\], ctions=\[partial\_count(1)\])
...
df\_joined = df1.join(broadcast(df2), "id")
